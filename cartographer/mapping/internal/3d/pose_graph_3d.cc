...
void PoseGraph3D::WaitForAllComputations() {
  ...
  // a WhenDone() callback.
  {
    const auto predicate =
        [this]() ABSL_EXCLUSIVE_LOCKS_REQUIRED(work_queue_mutex_) {
  ...
  const auto predicate = [&notification]() ABSL_EXCLUSIVE_LOCKS_REQUIRED(
                             mutex_) { return notification; };
...
void PoseGraph3D::AddSubmapFromProto(
  ...
  AddWorkItem(
      [this, submap_id, global_submap_pose]() ABSL_LOCKS_EXCLUDED(mutex_) {
        absl::MutexLock locker(&mutex_);
...
void PoseGraph3D::SetTrajectoryDataFromProto(
  ...
  AddWorkItem([this, trajectory_id,
               trajectory_data]() ABSL_LOCKS_EXCLUDED(mutex_) {
    absl::MutexLock locker(&mutex_);
    if (CanAddWorkItemModifying(trajectory_id)) {
